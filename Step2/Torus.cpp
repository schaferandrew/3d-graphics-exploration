...
void CTorus::Draw()
{
    ...
    for(int s=0;  s<m_steps1;  s++, a1a = a1b, a1b += step1r)
    {
        ...
        for(int s2=0;  s2<m_steps2;  s2++, a2a = a2b,  a2b += step2r)
        {
			...
			double u1 = atan2(v[0], v[2]) / (2. * GR_PI) + 0.5;
			double v1 = asin(v[1]) / GR_PI + .5;
			glTexCoord2d(u1, v1);	
            glNormal3dv(n);
            glVertex3dv(v);

            TorusVertex(a1b, m_r1, a2a, m_r2, v, n);
			double u2 = atan2(v[0], v[2]) / (2. * GR_PI) + 0.5;
			double v2 = asin(v[1]) / GR_PI + .5;			
			glTexCoord2d(u2, v2);
            glNormal3dv(n);
            glVertex3dv(v);
			...
			double u3 = atan2(v[0], v[2]) / (2. * GR_PI) + 0.5;
			double v3 = asin(v[1]) / GR_PI + .5;
			glTexCoord2d(u3, v3);
			glNormal3dv(n);
            glVertex3dv(v);

			TorusVertex(a1a, m_r1, a2b, m_r2, v, n);
			double u4 = atan2(v[0], v[2]) / (2. * GR_PI) + 0.5;
			double v4 = asin(v[1]) / GR_PI + .5;
			glTexCoord2d(u4, v4);
			glNormal3dv(n);
            glVertex3dv(v);
